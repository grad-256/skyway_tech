...
export class SkyWayConnectivityTest {
    ...
    try {
      ...
      return {
        ...
        recommendedConnectionType:
          this.determineRecommendedConnectionType(latency),
        connectionState
      }
    } catch (error) {
  ...
  private async testMediaDevices(): Promise<{
    audio: boolean
    video: boolean
  }> {
    try {
      // SkyWayStreamFactoryを使用
      const video = await SkyWayStreamFactory.createCameraVideoStream().catch(
        () => null
      )
      const audio =
        await SkyWayStreamFactory.createMicrophoneAudioStream().catch(
          () => null
        )
  ...
  private determineRecommendedConnectionType(
    latency: number
  ): "P2P" | "SFU" | null {
  ...
  private async cleanup(): Promise<void> {
    try {
      ...
      if (audioContainer) {
        const testAudios = audioContainer.querySelectorAll(
          'audio[data-publisher-id*="test-room"]'
        )
